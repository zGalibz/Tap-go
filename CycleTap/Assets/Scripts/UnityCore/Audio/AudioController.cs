...
namespace UnityCore
{
    namespace Audio
    {
        public class AudioController : MonoBehaviour
        {
            ...
            public float fade_duaration = 1.0f;
            public bool debug;
            public AudioTrack[] tracks;

            private Hashtable m_AudioTable;//relationship between audio types (key) and audio tracs (value)
            ...
            private void GenerateAudioTable()
            {
                foreach (AudioTrack _track in tracks)
                {
                    foreach (AudioObject _obj in _track.audio)
                    {
                        ...
                        {
                            LogWarning("you are trying to register audio [" + _obj.type + "] taht already been registered");
                        }
                        else
                        {
            ...
            private IEnumerator RunAudioJob(AudioJob _job)
            {
                ...
                if (_job.fade)
                {
                    ...
                    while (_timer <= fade_duaration)
                    {
                        _track.source.volume = Mathf.Lerp(_initial, _target, _timer / fade_duaration);
